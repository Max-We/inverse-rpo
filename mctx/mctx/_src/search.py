...
def welfords_update(val_parent, var_parent, n_parent, val_leaf):
    """Update running mean and variance using Welford's online algorithm.

    Args:
        val_parent: Current running mean
        var_parent: Current running variance
        n_parent: Current sample count
        val_leaf: New data point to incorporate

    Returns:
        tuple: Updated (mean, variance)
    """
    dtype = val_parent.dtype
    n_parent = jnp.asarray(n_parent, dtype=dtype)

    # Welford algorithm
    n_new = jnp.asarray(n_parent + 1, dtype=dtype)
    # 1. mean update
    delta = val_leaf - val_parent
    val_new = val_parent + delta / n_new
    # 2. variance update
    delta2 = val_leaf - val_new
    var_new = (var_parent * n_parent + delta * delta2) / n_new
...
def backward(
  ...
  def body_fun(loop_state):
    ...
    parent_value, parent_variance = welfords_update(
